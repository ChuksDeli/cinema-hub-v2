...
import { formatRating, getYear, getRatingColor, truncate } from '@/lib/utils';
...
export default function ShowDetailPage() {
  ...
  useEffect(() => {
    if (!id) return;
    tmdb.getTVDetails(id).then((data) => {
  ...
  const similar = (show as unknown as { similar?: { results: Movie[] } }).similar?.results?.slice(0, 12) || [];
  const trailer = show.videos?.results?.find((v) => v.type === 'Trailer' && v.site === 'YouTube');
  const creator = (show as unknown as { created_by?: { name: string }[] }).created_by?.[0];
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-24 relative z-10">
        <div className="flex flex-col md:flex-row gap-8 md:gap-12">
          ...
          >
            <div className="w-44 sm:w-52 md:w-64 mx-auto md:mx-0">
              {posterUrl ? (
                <Image src={posterUrl} alt={title} width={256} height={384} className="w-full border-2 border-border-default" />
              ) : (
          ...
          >
            <div className="inline-block bg-accent px-2 py-0.5 text-xs font-bold tracking-widest text-white mb-3">
              TV SERIES
            </div>
            <h1 className="font-display text-4xl sm:text-5xl md:text-6xl text-text-primary tracking-widest leading-none mb-2">
              {title}
            </h1>
            {(show as unknown as { tagline?: string }).tagline && (
              <p className="text-accent text-sm italic mb-4">&ldquo;{(show as unknown as { tagline: string }).tagline}&rdquo;</p>
            )}

 
            <div className="flex flex-wrap items-center gap-4 mb-6">
              <span className="text-text-secondary text-sm">{year}</span>
              {runtime && <span className="text-text-secondary text-sm">{runtime}m / episode</span>}
              ...
              {show.number_of_episodes && (
                <span className="text-text-secondary text-sm">{show.number_of_episodes} Episodes</span>
              )}
              <div
                className="px-2 py-0.5 text-xs font-bold text-bg-primary"
                style={{ backgroundColor: getRatingColor(show.vote_average) }}
              >
                {formatRating(show.vote_average)}
              </div>
              {show.status && (
                <span className="border border-border-default px-2 py-0.5 text-xs text-text-muted">{show.status}</span>
              )}
            </div>
            ...
            <p className="text-text-secondary leading-relaxed mb-6 max-w-2xl">{show.overview}</p>

            {creator && (
              <p className="text-text-secondary text-sm mb-2">
                <span className="text-text-muted mr-2">Creator</span>
            ...
            <StreamingPlatforms tmdbId={show.id} type="tv" title={title} />
          </motion.div>
        </div>
        ...
        <div className="mt-8 mb-12">
          <Link href="/tv" className="inline-flex items-center gap-2 text-text-muted hover:text-text-primary text-sm transition-colors">
